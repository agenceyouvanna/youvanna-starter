...
// ============================================
// 0. Résidus de clone (mail / SMTP / plugins désinstallés)
// ============================================
WP_CLI::log('── 0. Nettoyage résidus de clone ──');
...
$total_cleaned = 0;
foreach ($residue_patterns as $pattern) {
    $deleted = $wpdb->query($wpdb->prepare("DELETE FROM {$wpdb->options} WHERE option_name LIKE %s", $pattern));
    if ($deleted) {
        $total_cleaned += $deleted;
        WP_CLI::log("  supprimé {$deleted} option(s) correspondant à {$pattern}");
    }
}
delete_option('auto_core_update_notified');
delete_option('new_admin_email');
WP_CLI::log("Résidus options nettoyés: {$total_cleaned}");

// Config Resend mailer — destinataire par défaut = admin_email (Agent 0 peut l'override)
if (!get_option('yv_mail_to')) {
    update_option('yv_mail_to', get_option('admin_email'));
    WP_CLI::log('yv_mail_to initialisé = admin_email');
}
if (!get_option('yv_mail_bcc')) {
    update_option('yv_mail_bcc', 'agenceyouvanna@gmail.com');
    WP_CLI::log('yv_mail_bcc = agenceyouvanna@gmail.com');
}
if (!get_option('yv_mail_from_name')) {
    update_option('yv_mail_from_name', get_bloginfo('name'));
    WP_CLI::log('yv_mail_from_name = ' . get_bloginfo('name'));
}

// Vérif mu-plugin yv-resend-mailer présent
$resend_muplugin = WPMU_PLUGIN_DIR . '/yv-resend-mailer.php';
if (!file_exists($resend_muplugin)) {
    WP_CLI::warning('mu-plugin yv-resend-mailer.php manquant ! Les emails ne passeront pas par Resend.');
} else {
    WP_CLI::log('mu-plugin yv-resend-mailer présent.');
}

// Corrige les CF7 formulaires qui auraient un sender wordpress@<clonedomain> (résidu)
$cf7_forms = get_posts(['post_type' => 'wpcf7_contact_form', 'numberposts' => -1]);
foreach ($cf7_forms as $form) {
    $mail = get_post_meta($form->ID, '_mail', true);
    if (is_array($mail) && isset($mail['sender']) && strpos($mail['sender'], 'wordpress@') !== false) {
        $mail['sender'] = get_bloginfo('name') . ' <contact@youvanna.com>';
        update_post_meta($form->ID, '_mail', $mail);
        WP_CLI::log("  CF7 form #{$form->ID} sender corrigé");
    }
    if (is_array($mail) && isset($mail['recipient']) && in_array($mail['recipient'], ['lucile.perot.pro@gmail.com', 'demo@exemple.com'], true)) {
        $mail['recipient'] = get_option('yv_mail_to', get_option('admin_email'));
        update_post_meta($form->ID, '_mail', $mail);
        WP_CLI::log("  CF7 form #{$form->ID} recipient corrigé -> " . $mail['recipient']);
    }
}
